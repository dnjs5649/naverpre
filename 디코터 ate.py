...
def conv_block(nl,cc, input_embed,rate,is_train=True):
...
def conv_block1(nl,cc, input_embed,rate, is_train=True):
...
def infer_block(v,b,n):
    ...
    k=n
    cvs=conv_block(v,b,np.zeros([1,1,wdim]),a)
    co1 = tf.concat([k,cvs],1)
...
with tf.Session() as sess:
    ...
    for step in range(50001):
        if step <= 1000:
            p = sess.run(cost, feed_dict={X: data, Y: label12, is_training: True, Z: v,P:start})
            o, i = p
            print(step, o)
            if step % 50 == 0:
            ...
            p = sess.run(cost, feed_dict={X: data, Y: label12, is_training: True, Z: v1,P:start})
            o, i = p
            print(step, o)
            if step % 50 == 0:
                testp = sess.run(pr, feed_dict={X: datay, Y: labely12, is_training: False,P:start1})
